 ...
 * 
 *  @(#) $Id: jsp_staticapi.cpp 1 2008-08-22 05:57:26Z koban $
 */
...
DECLARE_API(CRE_TSK,"CRE_TSK")
{
    ...
    p.getToken(",","{",NULL);
    parseParameters(p,node,"tskatr,exinf,task,itskpri,stksz,stkdef");		// koban
	p.getToken("}",NULL);				// koban
	if( (*node)["stkdef"] == "NULL" )	// koban
		(*node)["stkdef"] = "";			// koban
...
DECLARE_API(CRE_DTQ,"CRE_DTQ")
{
    ...
    p.getToken(",","{",NULL);
    //parseParameters(p,node,"dtqatr,dtqcnt");	// 変更 koban 08/06/24
    //p.getToken(",","NULL","}",NULL);
    parseParameters(p,node,"dtqatr,dtqcnt,dtq");
    p.getToken("}",NULL);

	if( (*node)["dtq"] == "NULL" ) (*node)["dtq"] = "";		// koban
}
...
DECLARE_API(CRE_MPF,"CRE_MPF")
{
    ...
    p.getToken(",","{",NULL);
    //parseParameters(p,node,"mpfatr,blkcnt,blksz");	// 変更 koban 08/06/24
    //p.getToken(",","NULL","}",NULL);
    parseParameters(p,node,"mpfatr,blkcnt,blksz,limit");
    p.getToken("}",NULL);
	if( (*node)["limit"] == "NULL" ) (*node)["limit"] = "";		// koban
}
